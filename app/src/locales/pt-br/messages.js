/*eslint-disable*/ module.exports = {
  messages: {
    "$5 / Month": "5 dólar / mês",
    "$50 / Year": "50 dólares / Ano",
    "Access these charts from anywhere.<0/>Share/embed charts that stay in sync with your edits.":
      "Acesse os diagramas de qualquer lugar.<0/>Compartilhe/incorpore diagramas que ficam em sincronização com suas edições.",
    "Already a sponsor? Log in here": "Já é um patrocinador? Faça login aqui",
    Amount: "Total",
    "An error occurred. Try resubmitting or email {0} directly.": [
      "Ocorreu um erro. Tente reenviar ou envie um e-mail direto para ",
      ["0"],
      ".",
    ],
    Appearance: "Aparência",
    "Back To Editor": "Voltar ao editor",
    "Become a Sponsor": "Torne-se um(a) patrocinador(a)",
    "Bottom to Top": "De baixo para cima",
    Breadthfirst: "Por extensão",
    Cancel: "Cancelar",
    "Cancel your subscription. Your hosted charts will become read-only.":
      "Cancele sua inscrição. Seus diagramas hospedados não serão modificáveis.",
    Charts: "Diagramas",
    "Check your email for a link to log in. You can close this window.":
      "Cheque seu e-mail com um link para logar. Você pode fechar esta janela.",
    Circle: "Círculo",
    Clay: "Argila",
    Clear: "Apagar",
    "Clear text?": "Limpar o texto?",
    Clone: "Clonar",
    Close: "Fechar",
    "Comic Book": "História em Quadrinhos",
    Concentric: "Concêntrico",
    Contract: "Contrair",
    "Convert to hosted chart?": "Converter em diagrama hospedado?",
    Copy: "Copiar",
    "Copy PNG Image": "Copiar imagem PNG",
    "Copy SVG Code": "Copiar código SVG",
    Create: "Criar",
    "Create a link directly using the exact label text":
      "Crie uma conexão direta usando exatamente o mesmo texto da etiqueta",
    Created: "Criado",
    Dark: "Escuro",
    "Dark Mode": "Modo escuro",
    Date: "Data",
    Delete: "Deletar",
    Direction: "Direção",
    "Do you want to cancel your subscription?":
      "Você deseja cancelar sua inscrição?",
    "Do you want to delete this?": "Você deseja deletar isso?",
    Download: "Baixar",
    "Download JPG": "Baixar JPG",
    "Download PNG": "Baixar PNG",
    "Download SVG": "Baixar SVG",
    Duplicate: "Duplicar",
    "Edit your charts on any device. Share them with anyone. Publish your charts once and they remain up to date with your changes.":
      "Edite seus gráficos em qualquer dispositivo. Compartilhe-os com qualquer pessoa. Publique seus gráficos uma vez e eles permanecerão atualizados com suas alterações.",
    Editable: "Editável",
    Editor: "Editor",
    Eggs: "Ovos",
    Email: "E-mail",
    "Enter a title": "Insira um título",
    Excalidraw: "Excalidraw",
    Expand: "Expandir",
    Export: "Exportar",
    Feedback: "Feedback",
    "Follow Us": "Siga-nos",
    "Freeze Layout": "Travar Layout",
    "Freezing your chart allows you to customize the layout by disabling automatic layout.":
      "Congelar o seu gráfico permite que você personalize o layout desativando o layout automático.",
    Fullscreen: "Tela cheia",
    Futuristic: "Futurista",
    "Get More Layouts": "Obtenha Mais Leiautes",
    "Get More Themes": "Obtenha mais temas",
    "Get access to alternative styles for your flowcharts":
      "Obtenha acesso a estilos alternativos para seus fluxogramas",
    Graph: "Diagrama",
    Grid: "Grade",
    "Have fun! 🎉": "Divirta-se! 🎉",
    Help: "Ajuda",
    History: "Histórico",
    Home: "Página inicial",
    "Hosted Charts": "Diagramas hospedados",
    "Indenting creates a link to the current line":
      "Indentando cria uma conexão para com a linha atual",
    Language: "Idioma",
    Layout: "Layout",
    "Learn More": "Saber mais",
    "Left to Right": "Da esquerda para direita",
    Light: "Claro",
    "Light Mode": "Modo claro",
    Link: "Link",
    "Local Charts": "Diagramas locais",
    "Log Out": "Deslogar",
    "Made by <0>Tone Row</0>": "Feito por <0>Tone Row</0>",
    "Make a Donation": "Faça uma doação",
    "Make publicly accessible": "Tornar publicamente acessível",
    Monospace: "Monospace",
    "More Layouts": "Mais Layouts",
    "More Themes": "Mais temas",
    Museum: "Museu",
    Name: "Nome",
    "Need Help? <0>Check out our documentation.</0>":
      "Preciso de ajuda? <0>Confira nossa documentação.</0>",
    New: "Novo",
    "Next charge": "Próxima cobrança",
    Playbook: "Cartilha",
    "Powerful layout algorithms that bring order to graphs of all shapes and sizes":
      "Poderosos algoritmos de layout que trazem ordem para gráficos de todos os formatos e tamanhos",
    Public: "Público",
    Random: "Aleatório",
    "Read-only": "Somente leitura",
    Rename: "Renomear",
    Reset: "Resetar",
    "Resume Subscription": "Resumir inscrição",
    Retro: "Retrô",
    Return: "Retornar",
    "Right to Left": "Da direita para esquerda",
    Settings: "Configurações",
    "Sign In": "Entrar",
    "Sign Up": "Inscreva-se",
    "Sponsor flowchart.fun for $5 a month to access hosted flowcharts and the newest styles and features":
      "Patrocine flowchart.fun com $5 por mês para ter acesso à diagramasfluídos hospedados e os mais novos estilos e funcionalidades",
    "Sponsor flowchart.fun for <0>$5/month</0> or <1>$50/year</1> to get access to...":
      "Colabore com o flowchart.fun com <0>US$ 5,00 por mês</0> ou <1>US$ 50,00 por ano</1> para ter acesso a...",
    Sponsors: "Patrocínios",
    Start: "Início",
    Status: "Status",
    Submit: "Enviar",
    Subscription: "Inscrição",
    "Subscription will end": "Inscrição acabará",
    "Thank you for your feedback!": "Agradecimentos pelo seu feedback!",
    Theme: "Tema",
    "These charts are only available in this browser on this device.<0/>Clearing your browser local storage will erase these.":
      "Estes diagramas estão apenas disponíveis nesse navegador desse dispositivo.<0/>Limpando o local storage do seu navegador, irá apagá-los.",
    "This app works by typing": "Este aplicativo funciona por digitação",
    "Top to Bottom": "De cima para baixo",
    "Try again": "Tente novamente",
    Updated: "Atualizado",
    User: "Usuário",
    "View on Github": "Visualizar no Github",
    "We appreciate all of your feedback, suggestions, bugs, and feature requests!":
      "Nós agradecemos todos os seus feedbacks, sugestões, bugs, e requisições de funcionalidades!",
    "We use cookies to keep you logged in.":
      "Nós usamos cookies para manter você logado.",
    "What would you like to name this copy?":
      "Como você gostaria de chamar essa cópia?",
    "What would you like to share with us?":
      "O que você deseja compartilhar com a gente?",
    "Your subscription is no longer active. If you want to create and edit hosted charts become a sponsor.":
      "Sua inscrição não está mais ativa. Se você quer criar e editar seus diagramas hospedados torne-se um(a) patrocinador(a).",
    "[custom ID] or": "[ID customizada] ou",
    "any text: before a colon creates a label":
      "qualquer texto: antes de dois pontos cria uma etiqueta",
    "by adding an %5BID%5D and referencing that":
      "adicionando uma %5BID%5D e referenciando esta",
    comments: "comentários",
    "like this: (This app works by typing)":
      "tipo assim: (Este aplicativo funciona por digitação)",
    "like this: (custom ID) // You can also use single-line comments":
      "tipo assim: (ID customizada) // Você pode também usar comentários de uma linha só",
    multiline: "multilinha",
    or: "ou",
  },
};

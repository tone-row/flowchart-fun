...
describe("App", () => {
  ...
  test("SVG", async ({ page }) => {
    ...
    expect(buffer).toMatchSnapshot("test.svg", { threshold: 0.5 });
  });
  ...
  test("PNG", async ({ page }) => {
    ...
    const stream = await download.createReadStream();
    const buffer = await streamToBuffer(stream);
    expect(buffer).toMatchSnapshot("test.png", { threshold: 0.5 });
  });
  ...
  test("JPG", async ({ page }) => {
    ...
    const stream = await download.createReadStream();
    const buffer = await streamToBuffer(stream);
    expect(buffer).toMatchSnapshot("test.jpg", { threshold: 0.5 });
  });
});
